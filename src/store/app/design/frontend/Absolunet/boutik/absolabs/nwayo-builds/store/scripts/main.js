...
(function () {
	...
	//-- Subscribe to topics
	local.subscribe = function () {
		$.when(GLOBAL_JQUERY_LOAD).then(function () /* $Global */{
...
(function () {
	...
	local.bind = function () {
		...
		__.$window.on('resize', Foundation.util.throttle(function () {
		...
		__.$window.on('changed.zf.mediaquery', function (event, newSize, oldSize) {

			catalog.components.offcanvas.each(function (i, el) {
	...
	local.subscribe = function () {
		...
		$.when(GLOBAL_JQUERY_LOAD).then(function ($Global) {
			...
			$Global('body').on('contentUpdated', '#amasty-shopby-product-list', function (event) {
	...
	local.delayedStart = function () {
		...
		Foundation.onImagesLoaded($('.product-image-photo'), function () {
			__.$window.trigger('resize');
		});
